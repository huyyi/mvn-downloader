...
class MavenDownloader:
    def __init__(self, base_url="https://repo1.maven.org/maven2/", output_dir="./downloads", max_workers=10, index_mirrors=None, download_mirrors=None, verbose=False, exclude_patterns=None):
        ...
        # 配置索引镜像源列表（用于浏览目录列表）
        if index_mirrors is None:
            self.index_mirrors = [
                # 中科大镜像（支持 HTTP 目录浏览）
                "https://maven.proxy.ustclug.org/maven2/"
            ]
        else:
            self.index_mirrors = index_mirrors if isinstance(index_mirrors, list) else []

        # 配置下载镜像源列表（用于下载实际 artifact 文件）
        if download_mirrors is None:
            ...
            self.download_mirrors = download_mirrors if isinstance(download_mirrors, list) else []
    ...
    def _is_safe_segment(self, name):
        """判断单个路径片段是否安全（不包含跳转/多级路径）"""
        if not name:
            return False
        # 不允许包含路径分隔或向上跳转
    ...
    def _clean_href_dir(self, href):
        """清理目录型 href，返回安全的单段目录名，否则 None"""
        if not href:
            return None
        ...
        if not href.endswith('/'):
            return None
        dirname = href.rstrip('/')
        if not self._is_safe_segment(dirname):
            return None
        return dirname

    def _clean_href_file(self, href):
        """清理文件型 href，返回安全的单段文件名，否则 None"""
        if not href:
            return None
        href = unquote(href.strip())
        href = href.split('?', 1)[0].split('#', 1)[0]
        ...
        if href.endswith('/'):
            return None
        if not self._is_safe_segment(href):
            return None
        return href

    def _join_path(self, parent, child):
        """以安全方式拼接路径（不允许 child 为多级路径）"""
    ...
    def _try_request_with_mirrors(self, path, mirrors, timeout=30, stream=False, request_type="browse"):
        """使用指定的镜像列表请求，失败时回退到源站
        ...
        Args:
            ...
            timeout: 超时时间
            stream: 是否使用流式下载
            request_type: 请求类型（'browse' 用于浏览，'download' 用于下载，仅用于日志）
        ...
        try:
            ...
            error_type = "浏览" if request_type == "browse" else "下载"
            print(f"{error_type}失败（源站）: {path}, 错误: {e}")
            return None, None
    ...
    def try_request_with_mirrors(self, path, timeout=30, stream=False):
        ...
        Returns:
            (response, source_url) 或 (None, None)
        """
        return self._try_request_with_mirrors(path, self.index_mirrors, timeout, stream, request_type="browse")

    def download_with_mirrors(self, path, timeout=30, stream=False):
        """下载 artifact 文件时使用下载镜像源

        Args:
            path: 相对路径（如 org/springframework/boot/spring-boot-1.0.0.jar）
            timeout: 超时时间
            stream: 是否使用流式下载
        ...
        Returns:
            (response, source_url) 或 (None, None)
        """
        return self._try_request_with_mirrors(path, self.download_mirrors, timeout, stream, request_type="download")
    ...
    def get_artifacts_list(self, group_path):
        ...
        try:
            ...
            for link in soup.find_all('a'):
                href = link.get('href')
                item_name = self._clean_href_dir(href)
                if not item_name:
                    continue
                # 防护：如果项目名看起来像版本号（如 3.1.3），直接跳过，不做递归处理
                # 版本号应该在 artifact 内部处理，不应该作为 group 来递归
                if self._looks_like_version(item_name):
                    continue
                full_path = self._join_path(group_path, item_name)
    ...
    def _looks_like_version(self, name):
        ...
        if not name:
            return False
        # 如果开头是数字，则可能是版本号（如 1.2.3，2.0.0.RELEASE，2.0-beta-1）
        # 防止 3.1.3 这样的版本号被当作 group 来递归处理
        return name[0].isdigit()

    def _is_artifact_directory(self, path):
        """判断是否为 artifact 目录：优先依据 maven-metadata，其次看是否存在版本目录或 artifact 文件"""
        ...
        try:
            ...
            has_artifact_file = False
            for link in soup.find_all('a'):
                href = link.get('href')
                if not href:
                    continue
                # 只要存在 maven-metadata*（排除签名文件），即可认定为 artifact 目录
                if href and href.startswith('maven-metadata') and not href.endswith('.asc'):
                    return True
                # 检查是否存在 artifact 文件（.jar, .war, .pom 等）
                if href and any(href.endswith(ext) for ext in ['.jar', '.war', '.ear', '.pom']):
                    has_artifact_file = True
                # 检查是否存在版本目录
                dirname = self._clean_href_dir(href)
                if dirname and self._is_version_directory(dirname):
                    has_version_dir = True
            # 如果有 artifact 文件，说明这是版本目录或已发布的 artifact 目录，不应递归
    ...
    def get_versions_list(self, artifact_path):
        ...
        try:
            ...
            for link in soup.find_all('a'):
                href = link.get('href')
                version = self._clean_href_dir(href)
                if not version:
                    continue
                # 跳过 maven-metadata 等特殊目录
                if version.startswith('maven-metadata'):
                    continue
                versions.append(self._join_path(artifact_path, version))
    ...
    def get_files_in_version(self, version_path):
        ...
        try:
            ...
            for link in soup.find_all('a'):
                href = link.get('href')
                fname = self._clean_href_file(href)
                if not fname:
                    continue
                # 跳过哈希文件，但保留 maven-metadata.xml
                if fname.endswith(('.md5', '.sha1', '.sha256', '.sha512', '.asc')):
                    continue
                files.append(self._join_path(version_path, fname))
    ...
    def get_artifact_metadata(self, artifact_path):
        ...
        try:
            ...
            for link in soup.find_all('a'):
                ...
                if not fname:
                    continue
                if fname.startswith('maven-metadata') and not fname.endswith('.asc'):
                    metadata_files.append(self._join_path(artifact_path, fname))
    ...
    def download_file(self, file_path, progress_bar=None):
        ...
        response, source = self.download_with_mirrors(file_path, timeout=60, stream=True)
    ...
    def download_group(self, group_id, include_dependencies=True, max_depth=2, dry_run=False, _current_depth=0, _processed_groups=None):
        ...
        if _current_depth == 0:
            ...
            print(f"{indent}📑 索引镜像: {len(self.index_mirrors)} 个 + 源站")
            print(f"{indent}📥 下载镜像: {len(self.download_mirrors)} 个 + 源站")
...
def main():
    ...
    parser.add_argument('--index-mirrors', nargs='*', help='自定义索引镜像源列表（用于浏览目录，多个URL用空格分隔）')
    parser.add_argument('--download-mirrors', nargs='*', help='自定义下载镜像源列表（用于下载文件，多个URL用空格分隔）')
    parser.add_argument('--no-index-mirrors', action='store_true', help='不使用索引镜像源，仅用源站浏览')
    parser.add_argument('--no-download-mirrors', action='store_true', help='不使用下载镜像源，仅用源站下载')
    ...
    # 处理索引镜像源配置
    index_mirrors = None
    if args.no_index_mirrors:
        index_mirrors = []  # 空列表表示不使用镜像
    elif args.index_mirrors:
        index_mirrors = args.index_mirrors

    # 处理下载镜像源配置
    download_mirrors = None
    if args.no_download_mirrors:
        download_mirrors = []  # 空列表表示不使用镜像
    elif args.download_mirrors:
        download_mirrors = args.download_mirrors

    downloader = MavenDownloader(
        output_dir=args.output,
        max_workers=args.workers,
        index_mirrors=index_mirrors,
